...
function registerTsNode(projectRoot) {
  const tsNodeModule = __require.resolve("ts-node", {
    paths: [projectRoot]
  });
  try {
    __require(tsNodeModule).register({
      transpileOnly: true,
      files: false,
      compilerOptions: {
        module: "commonjs"
...
var AuthFactory = class {
  ...
  async saveSession(context, filePath, metadata = {}) {
    ...
    fs2.writeFileSync(filePath, JSON.stringify(enriched, null, 2), "utf-8");
    const payload = {
      filePath,
      enriched,
...
function getTokenFromFile(username, storageStatePath, config2, fallbackOrigin) {
  const filePath = path3.join(storageStatePath, `${username}.json`);
...
var AuthManager = class {
  ...
  async getContext(username, browser) {
    ...
    let state = null;
    try {
      const raw = fs4.readFileSync(storagePath, "utf-8");
...
async function deleteAuthStore(folderPath) {
  const absolutePath = path4.resolve(process.cwd(), folderPath);
  try {
...
async function getOrCreateAuthManager() {
  if (!authManagerInstance) {
    const _path = process.env.AUTH_USERS_FILE || path4.resolve(process.cwd(), "src/data/users.json");
...
async function ensureValidSession(username, browser) {
  ...
  const state = await fs6.readFile(file, "utf-8").catch(() => null);
  if (!state) {
