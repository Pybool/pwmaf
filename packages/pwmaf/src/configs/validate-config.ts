...
const VALID_TOKEN_STORAGE_TYPES = new Set(["localStorage", "sessionStorage"]);
...
function validateTokenStorageConfig(
  c: Collector,
  tokenConfig: TokenStorageConfig | undefined,
  prefix: string,
  user?: string,
): void {
  ...
  if (tokenConfig.tokenPath !== undefined && !tokenConfig.tokenPath.trim()) {
    c.warn(
      "EMPTY_TOKEN_PATH",
      `${prefix}.tokenPath`,
  ...
  if (
    tokenConfig.attachBearer !== undefined &&
    typeof tokenConfig.attachBearer !== "boolean"
  ) {
    c.error(
...
function validateOidcConfig(
  ...
  if (!oidc.provider) {
    c.error(
      "OIDC_CONFIG_PROVIDER_MISSING",
      `${prefix}.provider`,
      `oidcConfig.provider is required.`,
      `Valid values: ${[...VALID_OIDC].join(", ")}.`,
      undefined,
      user,
    );
  } else if (!VALID_OIDC.has(oidc.provider)) {
    c.error(
      "OIDC_CONFIG_INVALID_PROVIDER",
      `${prefix}.provider`,
      `oidcConfig.provider "${oidc.provider}" is not recognised.`,
      `Valid values: ${[...VALID_OIDC].join(", ")}.`,
      undefined,
      user,
    );
  ...
  if (oidc.issuer && !isValidUrl(oidc.issuer)) {
    c.error(
      "OIDC_CONFIG_INVALID_ISSUER",
      `${prefix}.issuer`,
      `oidcConfig.issuer "${oidc.issuer}" is not a valid URL.`,
      undefined,
      undefined,
      user,
    );
  ...
  if (oidc.redirectUri && !isValidUrl(oidc.redirectUri)) {
    c.error(
      "OIDC_CONFIG_INVALID_REDIRECT_URI",
      `${prefix}.redirectUri`,
      `oidcConfig.redirectUri "${oidc.redirectUri}" is not a valid URL.`,
      undefined,
      undefined,
      user,
    );
  ...
  if (oidc.mockServerUrl && !isValidUrl(oidc.mockServerUrl)) {
    c.error(
      "OIDC_CONFIG_INVALID_MOCK_SERVER_URL",
      `${prefix}.mockServerUrl`,
      `oidcConfig.mockServerUrl "${oidc.mockServerUrl}" is not a valid URL.`,
      undefined,
      undefined,
      user,
    );
...
function validateSingleUser(
  ...
  const effectiveTokenStorage =
    user.tokenStorageConfig ?? config.tokenStorageConfig;
  ...
  if (effectiveAuthType === "oidc" && user.oidcConfig) {
    validateOidcConfig(c, user.oidcConfig, `${label}.oidcConfig`, u);
  }
