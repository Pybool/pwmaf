...
import fs from "fs";
import { extractToken } from "../utils/tokenStorage";
...
export class AuthManager {
  ...
  async getContext(username: string, browser: PWBrowser): Promise<PWContext> {
    ...
    let state: EnrichedStorageState | null = null;
    try {
      const raw = fs.readFileSync(storagePath, "utf-8");
      state = JSON.parse(raw) as EnrichedStorageState;
    } catch {
      return browser.newContext();
    }
    ...
    const tokenStorage =
      user?.tokenStorageConfig ?? this.config.tokenStorageConfig ?? null;
    ...
    if (tokenStorage && state) {
      ...
      if (token) {
        ...
        const context = await browser.newContext({
          ...
          extraHTTPHeaders: {
            [headerName]:
              isAuthorization && attachBearer ? `Bearer ${token}` : token,
          },
        });
        await this.factory.restoreSessionStorage(context, state);
        return context;
      }
    }
