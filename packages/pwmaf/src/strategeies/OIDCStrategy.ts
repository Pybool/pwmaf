...
export class OIDCStrategy implements IAuthStrategy {
  async authenticate(
    ...
    try {
      ...
      // oidcConfig-first resolution with legacy fallbacks
      const oidcProvider: OIDCProvider | undefined =
        user.oidcConfig?.provider ?? user.oidcProvider ?? config.oidcProvider;

      const serverBase =
        user.oidcConfig?.mockServerUrl ??
        user.actionUrl ??
        config.BASE_SERVER_URL ??
        "http://localhost:3019";

      const callbackPath =
        user.oidcConfig?.redirectUri ?? `${serverBase}/auth/oidc/callback`;

      log(`loginUrl: ${loginUrl}`);
      log(`successUrl: ${successUrl}`);
      log(`provider: ${oidcProvider}`);
      log(`serverBase: ${serverBase}`);
      log(`callbackPath: ${callbackPath}`);
      ...
      await context.route(pattern, async (route) => {
        ...
        await route.fulfill({
          status: 302,
          headers: {
            location: `${callbackPath}?code=mock-oidc-code`,
          },
        });
      });
